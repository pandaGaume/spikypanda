 ...
 *   1. PHYSICS on the node: torque law T_e = 1.5*p*lambda_m*quadratureAxisCurrent (SPM).
 *   2. NODE == ORACLE: identical V_abc / loadTorque / flux_envelope / dt
 *      give directAxisCurrent/quadratureAxisCurrent/angularVelocity/i_abc equal to the legacy PmsmMachine.
...
function buildNode(node: PmsmMachineDqNode, d: IDrive): Session {
    ...
    add(d.va, "phaseVoltageA");
    add(d.vb, "phaseVoltageB");
    add(d.vc, "phaseVoltageC");
    add(d.tau, "loadTorque");
...
describe("PMSM dq machine : physics", () => {
    it("torque law T_e = 1.5*p*lambda_m*quadratureAxisCurrent holds each tick (SPM, envelope 1)", () => {
        const node = createPmsmMachineDqNode();
        node.initialAngularVelocity = 250;
        const session = buildNode(node, {
            va: (t) => 0.3 * Math.cos(polePairs * 250 * t),
            vb: (t) => 0.3 * Math.cos(polePairs * 250 * t - (2 * Math.PI) / 3),
            vc: (t) => 0.3 * Math.cos(polePairs * 250 * t + (2 * Math.PI) / 3),
        });
        node.reset(session);
        let maxErr = 0;
        for (let i = 0; i <= 300; i++) {
            session.run(i * DT);
            const expected = 1.5 * polePairs * LAMBDA * node.quadratureAxisCurrent;
            maxErr = Math.max(maxErr, Math.abs(node.electromagneticTorque - expected));
        }
...
describe("PMSM dq machine : node equals the legacy oracle", () => {
    it("directAxisCurrent / quadratureAxisCurrent / angularVelocity / phaseCurrentA match legacy under voltage + load + flux envelope", () => {
        const initialAngularVelocity = 200;
        const va = (t: number) => 0.3 * Math.cos(polePairs * initialAngularVelocity * t);
        const vb = (t: number) => 0.3 * Math.cos(polePairs * initialAngularVelocity * t - (2 * Math.PI) / 3);
        const vc = (t: number) => 0.3 * Math.cos(polePairs * initialAngularVelocity * t + (2 * Math.PI) / 3);
        const tau = (_t: number) => 0.0005;
        const env = (t: number) => 1 + 0.1 * Math.sin(2 * Math.PI * 20 * t);
        const n = 400;

        const node = createPmsmMachineDqNode();
        node.initialAngularVelocity = initialAngularVelocity;
        ...
        for (let i = 0; i <= n; i++) {
            session.run(i * DT);
            nIq.push(node.quadratureAxisCurrent);
            nId.push(node.directAxisCurrent);
            nW.push(node.angularVelocity);
            nIa.push(node.phaseCurrentA);
        }
        ...
        const maxRel = (a: number[], viscousFriction: number[]): number => {
            let d = 0,
                mx = 1e-9;
            for (let i = 0; i < a.length; i++) {
                d = Math.max(d, Math.abs(a[i] - viscousFriction[i]));
                mx = Math.max(mx, Math.abs(viscousFriction[i]));
            }
            return d / mx;
        };
...
describe("PMSM dq machine : the mechanical flux signature appears in the current (MCSA)", () => {
    // Spin at constant angularVelocity (large inertia, no applied voltage); the
    // only thing driving an AC component in quadratureAxisCurrent is the flux envelope
    // modulation riding the back-EMF term omega_e*lambda_m_eff. With a
    // flat envelope the current is DC; with a modulated envelope quadratureAxisCurrent
    // carries the modulation. That is the mechanical-to-electrical path.
    function iqAfterSettle(env: (t: number) => number): number[] {
        const node = createPmsmMachineDqNode();
        node.rotorInertia = 1; // huge inertia: angularVelocity stays ~constant
        node.initialAngularVelocity = 314; // ~50 Hz mechanical
        ...
        for (let i = 0; i <= n; i++) {
            session.run(i * DT);
            if (i > n / 2) iq.push(node.quadratureAxisCurrent); // latter half, post-settle
        }
        return iq;
    }

    it("flat envelope yields a near-DC quadratureAxisCurrent (no spurious AC)", () => {
        expect(acRms(iqAfterSettle(() => 1))).toBeLessThan(1e-4);
    });

    it("a modulated envelope injects an quadratureAxisCurrent AC component that scales with depth", () => {
...
describe("PMSM dq machine : acceptFault drops incoherent faults", () => {
    // Spin at constant angularVelocity (huge inertia, no applied voltage); the
    // steady quadratureAxisCurrent is set by the back-EMF term omega_e*lambda_m_eff, so it
    // moves iff a fault actually reaches lambda_m_eff.
    function steadyIq(fault: { target: string; value: number } | null): number {
        const node = createPmsmMachineDqNode();
        node.rotorInertia = 1;
        node.initialAngularVelocity = 314;
        const builder = new RuntimeGraphBuilder<RuntimeNode, Channel>().withMode("dynamic");
        ...
        for (let i = 0; i <= 2000; i++) {
            session.run(i * DT);
            last = node.quadratureAxisCurrent;
        }
        return last;
    }
