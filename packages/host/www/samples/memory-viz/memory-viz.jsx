...
// ============================================================
const { useState: useStateMV, useMemo: useMemoMV, useEffect: useEffectMV, useCallback: useCallbackMV } = React;
...
function MemoryViz() {
  ...
  const [step, setStep] = useStateMV(5);
  ...
  // All policy peaks for the comparator (use same order)
  ...
  if (!loaded) {
    return (
      <div className="h-full w-full flex flex-col">
        ...
        />
        <EmptyState onLoad={() => setLoaded(true)} theme={theme} />
      </div>
    );
  }
      ...
      <div className="max-w-lg w-full text-center">
        ...
        >
          ...
          <div className="flex flex-col items-start">
            <span className="text-sm">Charger un modèle ou glisser-déposer</span>
            <span className={`text-[11px] font-mono ${dark ? "text-neutral-600" : "text-neutral-500"}`}>.onnx (≤ 4 MB recommandé)</span>
          </div>
        </button>
