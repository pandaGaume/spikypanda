import { INeuron, ISnnGraph, ISpike, ISynapse } from "./spike.interfaces";
...
export class SNNRuntime {
     ...
     */
    public static prepareInputs(graph: ISnnGraph, inputData: number[]): Array<{ neuron: INeuron; amplitude: number }> {
     ...
     */
    public pulse(inputSpikes: Array<{ neuron: INeuron; amplitude: number }>) {
        ...
        while (activeSpikes.length > 0) {
            ...
            for (const spike of activeSpikes) {
                ...
                const synapses = neuron.onsc<ISynapse>(); // Ensure 'onsc' is an array
                if (synapses) {
                    for (const synapse of synapses) {
                        const postNeuron = synapse.ofin as INeuron;
     ...
     */
    private _fireNeuron(neuron: INeuron): ISpike {
        const spike: ISpike = {
            timestamp: Date.now(),
     ...
     */
    private _applySTDP(synapse: ISynapse, spikeTimestamp: number) {
        const preNeuron = synapse.oini as INeuron;
        const postNeuron = synapse.ofin as INeuron;
