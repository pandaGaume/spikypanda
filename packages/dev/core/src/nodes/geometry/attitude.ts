...
/**
 * Euler-angle source: holds yaw / pitch / roll in degrees as editable
 * properties and publishes the equivalent Quaternion on its output.
 *
 * Convention (matches Quaternion.fromYawPitchRoll):
 *   yaw   = rotation around Z (intrinsic, applied first)
 *   pitch = rotation around Y
 *   roll  = rotation around X (intrinsic, applied last)
 *   right-handed, X-forward / Y-right / Z-up.
 *
 * Degrees are stored on the model for friendly user editing; the
 * Quaternion getter converts to radians at read time.
 */
export class Attitude extends RuntimeNode implements IDeclaresPorts {
    ...
    /**
     * IDeclaresPorts contract: published so the editor can render
     * the three angle inputs and the rotation output without the
     * sample having to repeat the shape inline.
     */
    ...
    public readonly outputPorts: ReadonlyArray<IPortDescriptor> = [
        { slot: Attitude.OUTPUT_ROTATION, optional: false, type: "vec4" },
    ];
    ...
    public set yaw(v: number) {
        this.setField("yaw", this._yaw, v, (x) => { this._yaw = x; });
    }
    ...
    public set pitch(v: number) {
        this.setField("pitch", this._pitch, v, (x) => { this._pitch = x; });
    }
    ...
    public get rotation(): Quaternion {
        return Quaternion.fromYawPitchRoll(
            this._yaw   * DEG_TO_RAD,
            this._pitch * DEG_TO_RAD,
            this._roll  * DEG_TO_RAD,
        );
    }

    public override fire(session: ISession, _t: number): void {
        const { yaw, pitch, roll } = resolveSlotInputs(session, this, {
        ...
        publishToFirstOutput(session, this, Quaternion.fromYawPitchRoll(
            yaw * DEG_TO_RAD,
            pitch * DEG_TO_RAD,
            roll * DEG_TO_RAD,
        ));
