...
import type { ITensor, FFTEngine } from "spikypanda-core";
import { cloneable, editable, getFFTEngine } from "spikypanda-core";
import type { OnnxNodeInfo } from "../onnx-types";
import { OnnxOpNode, makeTensor, OnnxOpRegistry } from "../registry";

// The radix-2 FFT engine (FFTEngine + getFFTEngine cache) was extracted to
// `core/src/dsp/fft.ts` so it is a single, dependency-free brick shared by
// these onnx kernels and the spectral neural-network family. Imported above
// from spikypanda-core; the call sites below are unchanged.
