...
export class CoulombFrictionNode extends RuntimeNode implements IDeclaresPorts {
    ...
    public readonly outputPorts: ReadonlyArray<IPortDescriptor> = [
        { slot: "tau_friction", optional: false, type: "float" },
    ];
    ...
    @editable("number") public get tauC(): number { return this._tauC; }
    public set tauC(v: number) { this.setField("tauC", this._tauC, v, (n) => { this._tauC = n; }); }
    @editable("number") public get tauS(): number { return this._tauS; }
    public set tauS(v: number) { this.setField("tauS", this._tauS, v, (n) => { this._tauS = n; }); }
    @editable("number") public get omegaS(): number { return this._omegaS; }
    public set omegaS(v: number) { this.setField("omegaS", this._omegaS, v, (n) => { this._omegaS = n; }); }
    @editable("number") public get b(): number { return this._b; }
    public set b(v: number) { this.setField("b", this._b, v, (n) => { this._b = n; }); }
    @editable("boolean") public get opposesMotion(): boolean { return this._signSign; }
    public set opposesMotion(v: boolean) { this.setField("opposesMotion", this._signSign, v, (n) => { this._signSign = n; }); }

    @viewable("number") public get tau_friction(): number { return this._tau; }

    public override fire(session: ISession, _t: number): void {
        const links = session.graph.links as ReadonlyArray<IChannel>;
        let omega = 0, tauC = this._tauC, tauS = this._tauS;
        ...
        const stribeck = this._omegaS > 0
            ? tauC + (tauS - tauC) * Math.exp(-(omega * omega) / (this._omegaS * this._omegaS))
            : tauC;
        let tau = sign * stribeck + this._b * omega;
        if (this._signSign) tau = -tau;   // opposes motion

        this.setField("tau_friction", this._tau, tau, (n) => { this._tau = n; });
