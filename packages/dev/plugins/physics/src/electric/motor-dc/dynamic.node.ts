 ...
 */
export class DcMotorDynamicNode extends RuntimeNode implements IDeclaresPorts {
    // ── Physical parameters ─────────────────────────────────────────────
    ...
    @editable("number") public get i0(): number { return this._i0; }
    public set i0(v: number) { this.setField("i0", this._i0, v, (n) => { this._i0 = n; }); }
    ...
    public override reset(_session: ISession): void {
        this.setField("i",      this._i,     this._i0,     (n) => { this._i = n; });
        this.setField("omega",  this._omega, this._omega0, (n) => { this._omega = n; });
        this.setField("tau_em", this._tauEm, this._Kt * this._i0, (n) => { this._tauEm = n; });
        this._lastT = -1;
    }
    ...
    public override fire(session: ISession, t: number): void {
        ...
        // Read wired inputs (consume tokens; fall back to defaults).
        let V = 0, tauLoad = 0, dt = -1;
        ...
        if (dt > 0) {
            ...
            const dWdt = (this._Kt * this._i - this._b * this._omega - tauLoad) / Math.max(this._J, 1e-12);
            newI     = this._i     + dt * dIdt;
            newOmega = this._omega + dt * dWdt;
        }
        const newTauEm = this._Kt * newI;

        this.setField("i",      this._i,     newI,     (n) => { this._i = n; });
