 ...
 * SVPWM inverter node).
 */
export class PmsmMotorDynamicNode extends RuntimeNode implements IDeclaresPorts {
    ...
    public override fire(session: ISession, t: number): void {
        ...
        if (dt > 0) {
            ...
            newTauEm = Ke * this._P * (
                sinusoidalBackEmf(thetaE)                  * this._ia +
                sinusoidalBackEmf(thetaE + PHASE_OFFSET_B) * this._ib +
                sinusoidalBackEmf(thetaE + PHASE_OFFSET_C) * this._ic
            );
