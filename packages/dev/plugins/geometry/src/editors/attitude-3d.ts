...
export const attitude3DEditor: EditorFactory = (host, model): IEditor => {
    ...
    wrap.style.cssText =
        "display:flex;flex-direction:column;gap:10px;width:100%;height:100%;min-height:320px;";
    ...
    for (const axis of AXES) {
        ...
        row.style.cssText =
            ...
            "font-family:var(--ne-font-family,monospace);font-size:0.78em;" +
            "color:var(--ne-color-text);cursor:pointer;";
    ...
    canvas.style.cssText =
        "flex:1;min-height:240px;width:100%;border:1px solid var(--ne-color-border);" +
        "border-radius:4px;background:#0A0A0F;outline:none;cursor:ew-resize;touch-action:none;";
    canvas.tabIndex = 0;
    ...
    hint.style.cssText =
        "font-family:var(--ne-font-family,monospace);font-size:0.65em;" +
        "letter-spacing:0.04em;color:var(--ne-color-text-faint);text-align:right;";
    ...
    // Intentionally NOT attached: camera stays fixed across the session.
    ...
    drawAxis(scene, new Vector3(0, 0, 1), new Color3(0.25, 0.30, 0.65), "world-z");
    ...
    drawArrow(scene, body, new Vector3(1, 0, 0), new Color3(0.93, 0.30, 0.30), "body-x");
    drawArrow(scene, body, new Vector3(0, 1, 0), new Color3(0.30, 0.85, 0.35), "body-y");
    drawArrow(scene, body, new Vector3(0, 0, 1), new Color3(0.30, 0.55, 0.95), "body-z");
    ...
    // ── Resize handling ──────────────────────────────────────────────────
    const ro = new ResizeObserver(() => { engine.resize(); requestFrame(); });
    ro.observe(canvas);
...
function drawArrow(scene: Scene, parent: TransformNode, dir: Vector3, color: Color3, name: string): void {
    const shaft = MeshBuilder.CreateLines(name + "-shaft", {
