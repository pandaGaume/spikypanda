...
import { Cartesian3, editable, viewable, cloneable, IOlink, IDeclaresPorts, IPortDescriptor, ISession, publishToFirstOutput, resolveSlotInputs, RuntimeNode } from "spikypanda-core";
...
export class Cartesian3Node extends RuntimeNode implements IDeclaresPorts {
    ...
    public readonly outputPorts: ReadonlyArray<IPortDescriptor> = [
        { slot: Cartesian3Node.OUTPUT_VEC3, optional: false, type: "vec3" },
    ];
    ...
    public set x(v: number) { this.setField("x", this._x, v, (n) => { this._x = n; }); }

    @editable("number")
    public get y(): number { return this._y; }
    public set y(v: number) { this.setField("y", this._y, v, (n) => { this._y = n; }); }

    @editable("number")
    public get z(): number { return this._z; }
    public set z(v: number) { this.setField("z", this._z, v, (n) => { this._z = n; }); }
    ...
    public override fire(session: ISession, _t: number): void {
        const { x, y, z } = resolveSlotInputs(session, this, {
