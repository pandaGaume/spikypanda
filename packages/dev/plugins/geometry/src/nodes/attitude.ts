...
import { Quaternion, editable, viewable, cloneable, IOlink, IDeclaresPorts, IPortDescriptor, ISession, publishToFirstOutput, resolveSlotInputs, RuntimeNode, editor } from "spikypanda-core";
...
export class Attitude extends RuntimeNode implements IDeclaresPorts {
    ...
    public set yaw(v: number) { this.setField("yaw", this._yaw, v, (x) => { this._yaw = x; }); }

    @editable("number", { unit: "deg" })
    public get pitch(): number { return this._pitch; }
    public set pitch(v: number) { this.setField("pitch", this._pitch, v, (x) => { this._pitch = x; }); }

    @editable("number", { unit: "deg" })
    public get roll(): number { return this._roll; }
    public set roll(v: number) { this.setField("roll", this._roll, v, (x) => { this._roll = x; }); }
