/**
 * Compact, dependency-free, synchronous SHA-256 (FIPS 180-4) over a
 * Uint8Array, returning the digest as lowercase hex. Pure TypeScript:
 * no Node crypto, no WebCrypto, so it behaves identically under jest
 * (node) and inside the browser plugin bundle. Used by OnnxModelGraph
 * to verify model bytes pushed at runtime before they are allowed to
 * replace the live topology.
 *
 * Supports inputs up to 2^31 - 1 bytes, far beyond any .onnx payload
 * this plugin will ever receive.
 */

// Round constants: first 32 bits of the fractional parts of the cube
// roots of the first 64 primes (FIPS 180-4, section 4.2.2).
const K = new Uint32Array([
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5, 0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe,
    0x9bdc06a7, 0xc19bf174, 0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da, 0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
    0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967, 0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85, 0xa2bfe8a1, 0xa81a664b,
    0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070, 0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
    0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
]);

function hex8(v: number): string {
    return (v >>> 0).toString(16).padStart(8, "0");
}

/**
 * Compute the SHA-256 digest of `bytes` and return it as a 64-char
 * lowercase hex string.
 */
export function sha256Hex(bytes: Uint8Array): string {
    // Padding: append 0x80, zero-fill to a 64-byte boundary leaving 8
    // bytes for the 64-bit big-endian bit length trailer.
    const len = bytes.length;
    const blockCount = ((len + 8) >> 6) + 1;
    const padded = new Uint8Array(blockCount << 6);
    padded.set(bytes);
    padded[len] = 0x80;
    const dv = new DataView(padded.buffer);
    // (len << 3) keeps the low 32 bits of len * 8; the carry goes into
    // the high word (len / 2^29). Big-endian per the spec.
    dv.setUint32(padded.length - 8, Math.floor(len / 0x20000000), false);
    dv.setUint32(padded.length - 4, (len << 3) >>> 0, false);

    // Initial hash state: fractional parts of the square roots of the
    // first 8 primes (FIPS 180-4, section 5.3.3).
    let h0 = 0x6a09e667;
    let h1 = 0xbb67ae85;
    let h2 = 0x3c6ef372;
    let h3 = 0xa54ff53a;
    let h4 = 0x510e527f;
    let h5 = 0x9b05688c;
    let h6 = 0x1f83d9ab;
    let h7 = 0x5be0cd19;

    const w = new Uint32Array(64);
    for (let block = 0; block < padded.length; block += 64) {
        // Message schedule.
        for (let i = 0; i < 16; i++) {
            w[i] = dv.getUint32(block + (i << 2), false);
        }
        for (let i = 16; i < 64; i++) {
            const x = w[i - 15];
            const y = w[i - 2];
            const s0 = ((x >>> 7) | (x << 25)) ^ ((x >>> 18) | (x << 14)) ^ (x >>> 3);
            const s1 = ((y >>> 17) | (y << 15)) ^ ((y >>> 19) | (y << 13)) ^ (y >>> 10);
            w[i] = (w[i - 16] + s0 + w[i - 7] + s1) | 0;
        }

        // Compression.
        let a = h0;
        let b = h1;
        let c = h2;
        let d = h3;
        let e = h4;
        let f = h5;
        let g = h6;
        let h = h7;
        for (let i = 0; i < 64; i++) {
            const sig1 = ((e >>> 6) | (e << 26)) ^ ((e >>> 11) | (e << 21)) ^ ((e >>> 25) | (e << 7));
            const ch = (e & f) ^ (~e & g);
            const t1 = (h + sig1 + ch + K[i] + w[i]) | 0;
            const sig0 = ((a >>> 2) | (a << 30)) ^ ((a >>> 13) | (a << 19)) ^ ((a >>> 22) | (a << 10));
            const maj = (a & b) ^ (a & c) ^ (b & c);
            const t2 = (sig0 + maj) | 0;
            h = g;
            g = f;
            f = e;
            e = (d + t1) | 0;
            d = c;
            c = b;
            b = a;
            a = (t1 + t2) | 0;
        }
        h0 = (h0 + a) | 0;
        h1 = (h1 + b) | 0;
        h2 = (h2 + c) | 0;
        h3 = (h3 + d) | 0;
        h4 = (h4 + e) | 0;
        h5 = (h5 + f) | 0;
        h6 = (h6 + g) | 0;
        h7 = (h7 + h) | 0;
    }

    return hex8(h0) + hex8(h1) + hex8(h2) + hex8(h3) + hex8(h4) + hex8(h5) + hex8(h6) + hex8(h7);
}
