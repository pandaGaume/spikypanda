import { editable, viewable, cloneable, IOlink, IDeclaresPorts, IPortDescriptor, ISession, publishToFirstOutput, resolveSlotInputs, RuntimeNode } from "spikypanda-core";
...
abstract class BinaryCompareNode extends RuntimeNode implements IDeclaresPorts {
    ...
    public readonly outputPorts: ReadonlyArray<IPortDescriptor> = [{ slot: BinaryCompareNode.OUTPUT_RESULT, optional: false, type: "boolean" }];
...
export class EqualNode extends BinaryCompareNode {
    protected _predicate(a: number, b: number): boolean {
        return a === b;
    }
}
export class NotEqualNode extends BinaryCompareNode {
    protected _predicate(a: number, b: number): boolean {
        return a !== b;
    }
}
export class GreaterNode extends BinaryCompareNode {
    protected _predicate(a: number, b: number): boolean {
        return a > b;
    }
}
export class LessNode extends BinaryCompareNode {
    protected _predicate(a: number, b: number): boolean {
        return a < b;
    }
}
export class GreaterOrEqualNode extends BinaryCompareNode {
    protected _predicate(a: number, b: number): boolean {
        return a >= b;
    }
}
export class LessOrEqualNode extends BinaryCompareNode {
    protected _predicate(a: number, b: number): boolean {
        return a <= b;
    }
}
