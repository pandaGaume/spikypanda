...
DATA_PATH = REPORT_DIR / "data" / "trainable-receptive-fields-results.json"
OUTPUT_PATH = REPORT_DIR / "figures" / "09-trainable-receptive-fields-result.png"
...
def biquad_magnitude(frequencies_hz: np.ndarray, center_hz: float, bandwidth_hz: float) -> np.ndarray:
    ...
    z1 = np.exp(-1j * omega)
    numerator = b0 + b1 * z1 + b2 * z1**2
    denominator = 1 + a1 * z1 + a2 * z1**2
...
def main() -> None:
    ...
    baseline = data["referenceBaseline"]
    ...
    for index, field in enumerate(fields):
        color = colors[index]
        initial = biquad_magnitude(
            frequency_axis,
            field["initialCenterFrequencyHz"],
            field["initialBandwidthHz"],
        )
        final = biquad_magnitude(
            frequency_axis,
            field["learnedCenterFrequencyHz"],
            field["learnedBandwidthHz"],
        )
        axes[0, 0].plot(frequency_axis, initial, color=color, linestyle=":", alpha=0.8)
        axes[0, 0].plot(
            frequency_axis,
            final,
            color=color,
            linewidth=2,
            label=f"Champ {index + 1}: {field['learnedCenterFrequencyHz']:.3f} Hz",
        )
    axes[0, 0].set_title("Réponse fréquentielle des biquads", fontweight="bold")
    ...
    axes[0, 0].text(0.03, 0.05, "Pointillé: initial, trait plein: appris", transform=axes[0, 0].transAxes, fontsize=9)

    names = ["Baseline\n3 bandes fixes", "Champs\nrécepteurs appris"]
    validation_scores = [100 * baseline["validationAccuracy"], 100 * learned["validationAccuracy"]]
    test_scores = [100 * baseline["testAccuracy"], 100 * learned["testAccuracy"]]
    ...
    figure.suptitle("Capteur SNN à champs récepteurs fréquentiels appris", fontsize=14, fontweight="bold")
