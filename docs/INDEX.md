...
| [Sim Runtime: Integration Traits and Sample-Rate Aggregation](architecture/sim-runtime-and-sample-rates.md) | Sim-layer concepts above the dataflow core: `IIntegrable`, `IHasSampleRateRequirement`, the `IntegrableRuntimeNode` base class with `computeRequiredHz()` virtual hook + user-pin pattern, the `SimGraphNode` aggregation that turns leaf requirements into the inner session's `effectiveHz`, and the `disabledWhen` panel convention for read-only-when-wired editables. |
| [README](README.md) | Project overview, architecture summary, available modules |

## HELIOS

The HELIOS project (open source digital twin for closed-loop life support) has its own self-contained documentation directory at the repository root. It can be extracted as a standalone repository.

| Entry point | Description |
|---|---|
| [helios/README.md](../helios/README.md) | Autonomous HELIOS documentation set: project overview, embodied AI multi-level architecture, ISimGraph v2 framework design, 28-agent manifest, and website production materials (brief, copy, visuals specs). Self-contained, with its own reading order. |
